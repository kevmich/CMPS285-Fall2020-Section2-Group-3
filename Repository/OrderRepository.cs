    ...
    public class OrderRepository : IOrderRepository
    {
        ...
        public OrderView[] GetOrder(Guid guid)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var sql = $"SELECT Orders.Id, OrderItems.Name, Orders.Size, Orders.OrderItemId, Orders.IsDeleted FROM Orders INNER JOIN OrderItems ON Orders.OrderItemId = OrderItems.Id WHERE OrderNumber = @ordernumber ORDER BY DateStarted";
        ...
        public OrderViewKitchen[] GetUnfinishedOrders()
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var sql = $"SELECT OrderItems.Name, Orders.Size, Orders.OrderNumber, Orders.IsComplete, Orders.OrderItemId, Orders.IsDeleted FROM Orders INNER JOIN OrderItems ON Orders.OrderItemId = OrderItems.Id WHERE DateFinished IS NULL ORDER BY DateStarted";
